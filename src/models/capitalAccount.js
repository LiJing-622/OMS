...
import { query, queryUser, create, update, remove, batchRemove } from '../services/capitalAccount'
import { getLocalStorageJson } from '../utils/storage'

export default modelExtend(pageModel, {

  namespace: 'capitalAccount',
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        const match = pathToRegexp('/capitalAccount').exec(location.pathname)
        // const match2 = pathToRegexp('/capitalAccount/edit/:cdoe').exec(location.pathname)
        if (match) {
