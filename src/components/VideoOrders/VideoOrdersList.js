...
import { Table, Row, Col } from 'antd' // Popover,
  ...
  const columns = [
      ...
      render: (text, record) => {
        const content = (
          <div className={ListPopOverStyle}>
            ...
            <p>{`${record.patient.diagnose === null ? '无' : record.patient.diagnose}`}</p>
          </div>
        )
        return (
          content
          ...
          //             <div className={longText} style={{ textAlign: 'center', marginBottom: '1rem' }}>{`${record.patient.diagnose}`}</div>
          //             <div className={longText} style={{ textAlign: 'center', marginBottom: '1rem' }}>{`${record.patient.name}  ·  ${record.patient.sex === 0 ? '男' : '女'}  ·  ${record.patient.age}岁`}</div>
          //           </Popover>
          //         </div>
          //       </div>
          //     </div>}
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <span>
            <Link to={{ pathname: `/video-orders/edit/${orderNo}`, query: { currentList: pagination.current } }} >详情</Link>
